...
namespace KickSitePlugin
{
    ...
    class KickCommentProvider : ICommentProvider
    {
            ...
            try
            {
                switch (result.Command)
                {
                    ...
                    case "376":
                        ...
                        break;
                    default:
                        Debug.WriteLine($"Kick unknown command={result.Command}");
        ...
        private void OnMessageReceived(Result result)
        {
            ...
            var message = new KickComment(result.Raw)
            {
            ...
            var methods = new KickMessageMethods();
            var messageContext = new KickMessageContext(message, metadata, methods);
            MessageReceived?.Invoke(this, messageContext);
        }
        private void OnNoticeReceived(Result result)
        {
            var message = result.Params[1];
            var notice = new KickNotice(result.Raw)
            {
                Message = message,
            };
            ...
            var methods = new KickMessageMethods();
            var messageContext = new KickMessageContext(notice, metadata, methods);
            MessageReceived?.Invoke(this, messageContext);
        }
        ...
        {
            return _userStoreManager.GetUser(SiteType.Kick, userId);
        }
        ...
        private readonly KickSiteOptions _siteOptions;
        private readonly IUserStoreManager _userStoreManager;
        public KickCommentProvider(IDataServer server, ILogger logger, ICommentOptions options, KickSiteOptions siteOptions, IUserStoreManager userStoreManager)
        {
            _server = server;
            _logger = logger;
            _options = options;
